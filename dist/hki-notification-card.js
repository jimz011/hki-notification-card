      ...
      .confirmation-btn.confirm:active {
        transform: scale(0.97);
          ...
          ha-expansion-panel {
            border-radius: 12px;
            overflow: hidden;
            --expansion-panel-summary-padding: 14px 16px;
          }
          .panel-content {
            display: flex;
            flex-direction: column;
...
class HkiNotificationCardEditor extends LitElement {
  ...
  render() {
        ...
        ? html`
            <div class="side-by-side">
              ...
              ${this._config.show_empty
                ? this._renderInput("Empty Message", "empty_message", this._config.empty_message)
                : ""}
            </div>
          `
        : ""}
    ...
    const behaviorContent = html`
      ${mode === "marquee"
        ? html`
          ...
          `
        : mode === "list"
          ? html`${this._renderInput("Max Items Visible", "list_max_items", this._config.list_max_items, "number")}`
          : mode === "button"
            ? html`
                <div class="side-by-side">
                ...
                <ha-select
                  ...
                  @selected=${(e) => this._valueChanged(e, "alignment")}
                  @closed=${(e) => e.stopPropagation()}
                  fixedMenuPosition
                >
                  ${["left", "center", "right"].map(
                    (a) => html`<mwc-list-item .value=${a}>${a.charAt(0).toUpperCase() + a.slice(1)}</mwc-list-item>`
                  )}
                </ha-select>
              `
            : html`
                ...
                <div class="side-by-side">
                  <ha-select
                    ...
                    @selected=${(e) => this._valueChanged(e, "animation")}
                    @closed=${(e) => e.stopPropagation()}
                    fixedMenuPosition
                  >
                    ${[
                      ...
                      "elastic",
                      "swing",
                    ].map((a) => html`<mwc-list-item .value=${a}>${a.charAt(0).toUpperCase() + a.slice(1)}</mwc-list-item>`)}
                  </ha-select>
                  <ha-select
                    label="Direction"
                    .value=${this._config.direction || "right"}
                    @selected=${(e) => this._valueChanged(e, "direction")}
                    @closed=${(e) => e.stopPropagation()}
                    fixedMenuPosition
                  >
                    ${["left", "right", "top", "bottom"].map(
                      (d) => html`<mwc-list-item .value=${d}>From ${d.charAt(0).toUpperCase() + d.slice(1)}</mwc-list-item>`
                    )}
                  </ha-select>
                </div>
              `}
      ...
      ${mode === "button"
        ? html`
            ...
            ${this._config.button_label
              ? html`
                  <ha-select
                    label="Label Position"
                    .value=${this._config.button_label_position || "below"}
                    @selected=${(e) => this._valueChanged(e, "button_label_position")}
                    @closed=${(e) => e.stopPropagation()}
                    fixedMenuPosition
                  >
            ...
            ${this._config.button_show_badge !== false
              ? html`
                  <div class="side-by-side">
      ...
      mode === "button"
        ? html`
            ...
            <ha-select
              ...
              @selected=${(e) => this._valueChanged(e, "time_format")}
              @closed=${(e) => e.stopPropagation()}
              fixedMenuPosition
            >
            ...
            <p class="helper-text">
              Button mode always opens the popup when clicked. When "Confirm Tap Actions" is enabled, a confirmation dialog appears before executing any tap action.
            </p>
          `
        : html`
            <div class="side-by-side">
            ...
            ${this._config.popup_enabled !== false
              ? html`
                  <div class="side-by-side">
                  ...
                  ${!this._config.tap_action_popup_only
                    ? html`
                        ${this._renderSwitch("Confirm Tap Actions", "confirm_tap_action", this._config.confirm_tap_action)}
                    ...
                    : html`
                        <p class="helper-text">
                          When enabled, tapping always opens the popup first. Custom tap_actions from service calls will only work inside the popup.
                        </p>
                      `}
                `
              : html`<p class="helper-text">When enabled, tapping a notification opens a popup with all notifications and a clear button.</p>`}
          `;
    ...
    const layoutContent = html`
      ${mode !== "button"
        ? html`
            ...
            ${this._config.show_background !== false
              ? html`
                  <div class="side-by-side">
    ...
    const typographyContent = html`
      <div class="side-by-side">
        ...
        <ha-select
          ...
          @selected=${(e) => this._valueChanged(e, "font_weight")}
          @closed=${(e) => e.stopPropagation()}
          fixedMenuPosition
        >
      ...
      <ha-select
        label="Font Family"
        .value=${fontFamily}
        @selected=${(e) => this._valueChanged(e, "font_family")}
        @closed=${(e) => e.stopPropagation()}
        fixedMenuPosition
      >
    ...
    const headerContent = html`
      ...
      ${this._config.use_header_styling
        ? html`
    ...
    return html`
      <div class="card-config">
        ...
        ${this._renderPanel("Layout & Visibility", basicsContent, true)}
        ${mode === "button"
          ? this._renderPanel("Button Styling", behaviorContent, false)
          : this._renderPanel("Behavior", behaviorContent, false)}
        ${this._renderPanel("Popup", popupContent, false)}

        ${mode !== "button"
          ? html`
              ${this._renderPanel("Header Styling", headerContent, false)}
              ${!this._config.use_header_styling ? this._renderPanel("Appearance", layoutContent, false) : ""}
              ${!this._config.use_header_styling ? this._renderPanel("Typography", typographyContent, false) : ""}
            `
          : html`${this._renderPanel("Appearance", layoutContent, false)}`}
  ...
  _renderPanel(title, content, expanded = false) {
    return html`
      <ha-expansion-panel header="${title}" ?expanded=${expanded}>
        <div class="panel-content">${content}</div>
      </ha-expansion-panel>
    `;
  }

  _renderEntityPicker(label, field, value, helper = "", includeDomains = null, prefix = null) {
      const all = this.hass ? Object.keys(this.hass.states || {}) : [];
      if (prefix) {
        const items = all.filter((e) => e.startsWith(prefix));
        if (items.length) {
          return html`
            <ha-combo-box
              .label=${label}
              ...
              allow-custom-value
              @value-changed=${(ev) =>
                this._fireChanged({ ...this._config, [field]: ev.detail.value })}
            ></ha-combo-box>
          `;
        }
      }
  ...
  static get styles() {
      return css`
          ...
          ha-expansion-panel { border-radius: 12px; overflow: hidden; }
          .panel-content { display: flex; flex-direction: column; gap: 16px; padding: 0 16px 16px 16px; }
          .side-by-side { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
          .three-col { grid-template-columns: 1fr 1fr 1fr; }
          ha-textfield, ha-select, ha-selector, ha-combo-box { width: 100%; display: block; }
